from __future__ import annotations

from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field
from typing_extensions import Self

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.builtin_function_info_parameters_schema import (
        BuiltinFunctionInfoParametersSchema,
    )


T = TypeVar("T", bound="BuiltinFunctionInfo")


@_attrs_define
class BuiltinFunctionInfo:
    """
    Attributes:
        name (str):
        description (str):
        parameters_schema (BuiltinFunctionInfoParametersSchema | Unset):
    """

    name: str
    description: str
    parameters_schema: BuiltinFunctionInfoParametersSchema | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        name = self.name

        description = self.description

        parameters_schema: dict[str, Any] | Unset = UNSET
        if not isinstance(self.parameters_schema, Unset):
            parameters_schema = self.parameters_schema.to_dict()

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "name": name,
                "description": description,
            }
        )
        if parameters_schema is not UNSET:
            field_dict["parameters_schema"] = parameters_schema

        return field_dict

    @classmethod
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        from ..models.builtin_function_info_parameters_schema import (
            BuiltinFunctionInfoParametersSchema,
        )

        d = dict(src_dict)
        name = d.pop("name")

        description = d.pop("description")

        _parameters_schema = d.pop("parameters_schema", UNSET)
        parameters_schema: BuiltinFunctionInfoParametersSchema | Unset
        if isinstance(_parameters_schema, Unset):
            parameters_schema = UNSET
        else:
            parameters_schema = BuiltinFunctionInfoParametersSchema.from_dict(
                _parameters_schema
            )

        builtin_function_info = cls(
            name=name,
            description=description,
            parameters_schema=parameters_schema,
        )

        builtin_function_info.additional_properties = d
        return builtin_function_info

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
